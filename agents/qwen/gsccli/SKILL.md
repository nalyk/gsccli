...
> **Qwen Code caveats:** Qwen has an open issue (#2343) where skills sometimes don't auto-discover; if this skill isn't picked up, tell the user to run `/skills` once to load it manually. Qwen OAuth free tier was discontinued 2026-04-15 — the user must have a `DASHSCOPE_API_KEY`, an Alibaba Cloud Coding Plan, or an OpenAI-compatible endpoint configured. For tool-use reliability, `qwen3-coder-plus` (production) or `qwen3-coder-next` (local) are recommended; `qwen3-coder-flash` works but breaks with many tools loaded.
