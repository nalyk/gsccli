...
when_to_use: |
  User asks about: querying Google Search Console; analyzing search performance
  (clicks/impressions/CTR/position); finding keywords or pages losing clicks;
  comparing date periods; inspecting URL indexing status (single or batch);
  managing sitemaps; sending Indexing API notifications for job-postings or
  livestream content.
user_invocable: true
allowed_tools:
  - run_shell_command
...
- `unauthorized` / `invalid_grant` → tell the user to run `gsccli auth login` once in a shell (browser flow). Note: Gemini's sandbox modes block browser launch, so this MUST be done outside the sandbox.
